...
>  **EA**：CPU的总中断允许控制位，**EA=1，CPU开放中断**，EA=0，CPU屏蔽所有的中断申请。EA的作用是使中断允许形成两极控制。即各中断源首先受EA控制;其次还受各中断源自己的中断允许控制位控制。
...
void Timer0_Init() //11.0592GHZ
{
	...
	TMOD=TMOD&0xF0;   //把TMOD的低四位清零，高四位保持不变，不影响定时器1
	TMOD=TMOD|0x01;   //把TMOD最低位置1，高四位保持不变，设置定时器0为开启状态
	TF0=0;           //将溢出中断标志清0，便于后续检测
	TR0=1;           //开始计时
...
![image-20241105234408586](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20241105234408586.png)
 ...
 **停止位**：用于数据帧间隔
 ...
 ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/8aac400b9f4140f28efd80691ce82fa0.png)

#### 串口模式图

![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/1c8ca2eecff344378413a06b80ec513f.png)
 左边为总线，数据只有到了总线，CPU才能进行相关操作。
 数据通过TXD发出；数据通过RXD进入。

**SBUF**：**串口数据寄存器**，物理上是两个单独的寄存器，但占用相同的地址。写操作时，写入的是发送寄存器，读操作时，独处的是接收寄存器。

#### 串口相关寄存器
 ![在这里插入图片描述|300](https://i-blog.csdnimg.cn/direct/00171e91fb714bd8af20c271ca6c2d0e.png)

![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/004f4b767ccc4e1da738214927f470cc.png)



> SCON：串行控制寄存器
>
> ![这里是引用](https://i-blog.csdnimg.cn/direct/8885bf95f6a943e39113aea0b735711f.png)
>
...
> TI：**发送中断标志**。发送结束，内部硬件自动将T1置1，向主机请求中断，响应结束后，必须用软件复位T1，使T1置0。
> RI：接收中断请求标志。和T1类似。
>
> ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/8ede9d454d654552962fd0a3a410261d.png)

> **PCON：电源控制寄存器**
>
> <img src="https://i-blog.csdnimg.cn/direct/ef727a06914a4d9db3752a437ee2a030.png" alt="这里是引用" style="zoom:67%;" />
>
